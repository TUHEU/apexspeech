...
class DatabaseHelper {
  ...
  Future<Map<String, dynamic>?> getRecordingById(int id) async {
    final db = await database;
    final results = await db.query('recordings', where: 'id = ?', whereArgs: [id]);
    return results.isNotEmpty ? results.first : null;
  }

  Future<int> updateRecording(int id, Map<String, dynamic> data) async {
    final db = await database;
    return await db.update('recordings', data, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> getRecentRecordings(int limit) async {
    final db = await database;
    return await db.query('recordings', orderBy: 'date_created DESC', limit: limit);
  }
  ...
  Future<int> updateProfile(int id, Map<String, dynamic> data) async {
    final db = await database;
    return await db.update('profile', data, where: 'id = ?', whereArgs: [id]);
  }
}
